            ...
            <div class="layui-card">
                <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                    <div class="layui-form-item">
                        ...
                        <div class="layui-inline">
                            <label class="layui-form-label" style="font-weight: bold;">来源类型</label>
                            <div class="layui-input-block">
                                <select name="type" id="type">
                                    <option value="0">不限类型</option>
                                    <option value="1">学习所得</option>
                                    <option value="2">用户充值</option>
                                    <option value="3">用户消费</option>
                                </select>
                            </div>
                        </div>

                        <div class="layui-inline">
                            <label class="layui-form-label" style="font-weight: bold;">变动时间</label>
                            <div class="layui-input-inline">
                                <input type="text" name="time" id="time" placeholder="请选择变动时间" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="buttonTpl">
                        {{#  if(d.type == 1){ }}
                        <button class="layui-btn layui-btn-normal layui-btn-xs">用户充值</button>
                        {{#  } else if(d.type == 2){ }}
                        <button class="layui-btn layui-btn-danger layui-btn-xs">用户消费</button>
                        {{#  } else { }}
                        <button class="layui-btn layui-btn-xs">学习所得</button>
                        {{#  } }}
                    </script>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#OnlineLearning',
            url: '/points?method=queryAllPoints',
            cols: [[
                {type: 'checkbox', fixed: 'left'},
                {field: 'id', width: 80, title: '编号', sort: true, fixed: 'left', align: "center"},
                {field: 'userId', width: 120, title: '用户编号', align: "center"},
                {field: 'type', width: 120, title: '来源类型', sort: true, templet: "#buttonTpl", align: "center"},
                {field: 'number', width: 110, title: '积分数量', sort: true, align: "center"},
                {field: 'time', width: 160, title: '变动时间', sort: true, align: "center"},
            ...
            if (obj.event === 'detail') {
                ...
                layer.confirm('真的要删除这条数据吗?', function (index) {
                    ...
                    $(document).ready(function () {
                        $.ajax({
                            type: 'post',
                            url: '/points?method=deletePoints',
                            data: {'id': obj.data.id},
                            dataType: 'json',
                ...
                layer.open({
                    type: 2,
                    title: '编辑积分数据',
                    content: '/views/points/pointsEdit.jsp',
                    maxmin: true,
                    area: ['450px', '620px'],
                    btn: ['确定', '取消'],
        ...
        let active = {
            ...
            add: function () { // 新增数据(改这里)
                layer.open({
                    type: 2,
                    title: '添加订单数据',
                    content: '/views/points/pointsInsert.jsp',
                    maxmin: true,
                    area: ['450px', '620px'],
                    btn: ['确定', '取消'],
                    yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                            ...
                            $.ajax({
                                type: 'post',
                                url: '/points?method=insertPoints',
                                data: data.field,
                                dataType: 'json',
                                success: function (data) {
                                    if (data.result === 1) {
