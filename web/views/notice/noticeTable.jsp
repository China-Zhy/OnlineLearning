...
<head>
    ...
    <link rel="icon" href="/layuiadmin/resource/image/hmgcon.png">
            ...
            <div class="layui-card">
                <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            <label class="layui-form-label" style="font-weight: bold;">公告标题</label>
                            <div class="layui-input-block">
                                <input type="text" name="title" id="title" placeholder="请输入公告标题" autocomplete="off" class="layui-input" style="letter-spacing: 1px;">
                            </div>
                        </div>
                        <div class="layui-inline">
                            <label class="layui-form-label" style="font-weight: bold;">公告类型</label>
                            <div class="layui-input-block">
                                <select name="type" id="type">
                                    <option value="1">选择类型</option>
                                    <option value="0">系统公告</option>
                                    <option value="-1">课程公告</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="create" id="LAY-component-form-group-create" placeholder="请选择创建时间" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
                            <label class="layui-form-label" style="font-weight: bold;">截止时间</label>
                            <div class="layui-input-inline">
                                <input type="text" name="dateline" id="LAY-component-form-group-dateline" placeholder="请选择截止时间" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="buttonTpl">
                        {{#  if(d.target == 0){ }}
                        <button class="layui-btn layui-btn-xs">系统公告</button>
                        {{#  } else { }}
                        <button class="layui-btn layui-btn-checked layui-btn-xs">课程公告</button>
                        {{#  } }}
                    </script>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#OnlineLearning',
            url: '/notice?method=queryAllNotice',
            cols: [[
                {type: 'checkbox', fixed: 'left'},
                {field: 'id', width: 80, title: 'ID', sort: true, align: 'center', fixed: 'left'},
                {field: 'title', width: 160, title: '公告标题', align: 'center'},
                {field: 'info', width: 250, title: '公告内容', align: 'center'},
                {field: 'create', width: 160, title: '创建时间', align: 'center', sort: true},
                {field: 'dateline', width: 160, title: '截止时间', align: 'center', sort: true},
                {field: 'target', width: 120, title: '公告类型', align: 'center', sort: true, templet: "#buttonTpl"},
                {field: 'userId', width: 120, title: '创建用户', align: 'center', sort: true},
...
// 日期控件
        laydate.render({
            elem: '#LAY-component-form-group-create'
        });
        laydate.render({
            ...
            if (obj.event === 'detail') {
                ...
                layer.confirm('真的要删除这条数据吗?', function (index) {
                    $(document).ready(function () {
                        $.ajax({
                            type: 'post',
                            url: '/notice?method=deleteNotice',
                            data: {'id': obj.data.id},
                            dataType: 'json',
                ...
                layer.open({
                    type: 2,
                    title: '编辑公告',
                    content: '/views/notice/noticeEdit.jsp',
                    maxmin: true,
                    area: ['500px', '700px'],
                    btn: ['确定', '取消'],
                    ...
                    yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                            ...
                            $.ajax({
                                type: 'post',
                                url: '/notice?method=updateNotice',
                                data: data.field,
                                dataType: 'json',
                                success: function (data) {
                                    if (data.result === 1) {
        ...
        let active = {
            ...
            add: function () { // 新增数据(改这里)
                layer.open({
                    type: 2,
                    title: '添加公告数据',
                    content: '/views/notice/noticeInsert.jsp',
                    maxmin: true,
                    area: ['500px', '700px'],
                    btn: ['确定', '取消'],
                    yes: function (index, layero) {
...
// 按钮事件相关的控制(勿改)
        ...
        $('#clear').click(function () {
            $('#title').val('');
            $('#type').val('');
            $('#LAY-component-form-group-create').val('');
            $('#LAY-component-form-group-dateline').val('');
